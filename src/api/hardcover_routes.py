...
def api_hardcover_resolve():
    ...
    hardcover_client = _container.hardcover_client()
...
def link_hardcover(abs_id):
    ...
    if request.is_json:
        ...
        try:
            ...
            _database_service.save_hardcover_details(hardcover_details)

            # Force status to 'Want to Read' (1)
            try:
                _container.hardcover_client().update_status(
    ...
    # Resolve book
    book_data = _container.hardcover_client().resolve_book_from_input(url)
    if not book_data:
